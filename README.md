...
- **Concurrent, quality-matched downloads:** up to 8 tracks download at once (configurable via `MAX_CONCURRENT_DOWNLOADS`), each re-encoded at its actual source bitrate (capped at 320kbps) instead of a fixed rate.
